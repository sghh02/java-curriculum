# さまざまな種類のクラス

> 提出ブランチ：`feature/27-practice-class-types`（PRのbase：`develop`）

## ゴール

- この章の課題（練習問題/ハンズオン）を完了できる
- 章で扱ったテーマを、自分の言葉で3行で説明できる
- 作業ブランチ `feature/27-practice-class-types` で作業し、PRで提出できる

## 課題：第2章 さまざまな種類のクラス

この章を学習したら、以下の練習問題に取り組んでください。

## 練習問題1

以下の仕様に従った金庫をStrongBoxクラスとして定義してください。

- 金庫クラスに格納するインスタンスの型は、開発時には未定である。
- 金庫には、1つのインスタンスを保存できる必要がある。
- put()でインスタンスを保存し、get()でインスタンスを取得できる。
- get()で取得する際、キャストを使わなくても格納前の型に代入できる。

## 練習問題2

練習問題1で作成したStrongBoxクラスに鍵の種類を示す列挙型KeyTypeを定義した上で、以下の2つをStrongBoxクラスの定義に加えてください。

- カギの種類を示すフィールド
- 鍵の種類を受け取るコンストラクタ

ただし、鍵の種類は以下の4種類に限定されるものとします。

| ①南京錠(PADLOCK) | 必要施行回数=1,024回 |
| --- | --- |
| ②押ボタン(BUTTON) | 必要施行回数=10,000回 |
| ③ダイヤル (DIAL) | 必要施行回数=30,000回 |
| ④指紋認証(FINGER) | 必要施行回数= 1,000,000回 |

なお、金庫はget()が呼び出されるたびに回数をカウントし、各鍵が定める必要施行回数に到達しない限りnullを返すようにしてください。

## AIに質問する（この章の例）

次の例をそのままAIに投げてOKです（必要なら自分のコード/エラーに置き換えてください）。

```text
「『さまざまな種類のクラス』を初心者向けに、たとえ話つきで説明して。最後に要点を3つにまとめて」
「この章の内容で理解確認クイズを5問作って（解答は最後にまとめて）」
「この章の練習問題で詰まった。答えを丸ごとではなく、方針→ヒント→確認質問の順で教えて」
「この章の内容を、制作物（ToDo管理）にどう活かすか提案して」
```

## 完了条件（チェックリスト）

- [ ] 課題（練習問題/ハンズオン）の要件を満たした
- [ ] 自分で動作確認できた（実行結果/スクショ/ログのいずれか）
- [ ] `feature/27-practice-class-types` で作業し、コミットしてpushした
- [ ] `feature/27-practice-class-types` → `develop` のPRを作成した
- [ ] AI総合レビューツールでレビューし、指摘を反映した（または理由を説明できる）

---

## 課題提出

この章には提出課題があります。

1. 上記の練習問題を完了する
2. GitHub で `feature/27-practice-class-types` ブランチを作成し、PRを作成
3. [AI総合レビューツール](https://ai.studio/apps/drive/1AMqIqU4Bio4te7AWh5dly1Qzp7CesqP9?fullscreenApplet=true) でレビューを実行
4. 問題がなければ、スプレッドシートに **PR URL** と **完了日** を記入
